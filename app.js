...
// ===== LISTAS NEGRAS EN LOCALSTORAGE (persistencia de borrados) =====
const BORRADOS_PEDIDOS_KEY   = 'pedidos_borrados';
const BORRADOS_FALTANTES_KEY = 'faltantes_borrados';

function getPedidosBorrados() {
  try { return new Set(JSON.parse(localStorage.getItem(BORRADOS_PEDIDOS_KEY) || '[]')); }
  catch(e) { return new Set(); }
}
function addPedidoBorrado(factura) {
  try {
    const s = getPedidosBorrados(); s.add(factura);
    localStorage.setItem(BORRADOS_PEDIDOS_KEY, JSON.stringify([...s]));
  } catch(e) {}
}
function getFaltantesBorrados() {
  try { return JSON.parse(localStorage.getItem(BORRADOS_FALTANTES_KEY) || '[]'); }
  catch(e) { return []; }
}
function addFaltanteBorrado(factura, sku) {
  try {
    const arr = getFaltantesBorrados();
    const fNorm = factura.toString().trim();
    const sNorm = sku.toString().trim();
    if (!arr.find(x => x.f === fNorm && x.s === sNorm))
      arr.push({ f: fNorm, s: sNorm });
    localStorage.setItem(BORRADOS_FALTANTES_KEY, JSON.stringify(arr));
    console.log('[borrarFaltante] guardado en localStorage:', fNorm, sNorm, arr);
  } catch(e) { console.error('addFaltanteBorrado error:', e); }
}
function esFaltanteBorrado(factura, sku) {
  const fNorm = factura.toString().trim();
  const sNorm = sku.toString().trim();
  const result = getFaltantesBorrados().some(x => x.f === fNorm && x.s === sNorm);
  return result;
}
