...
function addFaltanteBorrado(factura, sku) {
  try {
    ...
    if (!arr.find(x => x.f === factura && x.s === sku))
      arr.push({ f: factura, s: sku });
    localStorage.setItem(BORRADOS_FALTANTES_KEY, JSON.stringify(arr));
  } catch(e) {}
}
function esFaltanteBorrado(factura, sku) {
  return getFaltantesBorrados().some(x => x.f === factura && x.s === sku);
}
  ...
  body.innerHTML = rows.map(r => `<tr>
    ...
    <td>
      <button class="btn-borrar-dup" title="Borrar este faltante"
        onclick="borrarFaltante('${escHtml(r.Factura)}', ${r._itemIndex})">✕</button>
    </td>
  </tr>`).join('');
}
...
async function toggleEnviado(factura, itemIndex, checked) {
  ...
  if (checked) {
    // Auto-asignar fecha actual si no tiene
    if (!item.FechaEnvio) {
      item.FechaEnvio = new Date().toISOString().slice(0,10);
    }
    showToast(`✅ ${item.NombreProducto} marcado como enviado`, 'success');
  }
  renderInforme(); // re-render inmediato — el item desaparece si Enviado=true
  await persistEnvioUpdate(factura, item);
}

async function borrarFaltante(factura, itemIndex) {
  try { await pedirClave('Borrar faltante', `Factura: ${factura}`); } catch(e) { return; }
  if (!despachados[factura] || !despachados[factura].items[itemIndex]) return;
  const item = despachados[factura].items[itemIndex];
  ...
  addFaltanteBorrado(factura, item.Sku);
  // Marcar en Sheets como backup (best effort)
  scriptPost('marcarEliminadoFaltante', [[factura, item.Sku, 'P', 'X']]).catch(()=>{});
