...
async function borrarPedido(factura) {
  ...
  // localStorage PRIMERO — persiste siempre aunque el Apps Script no responda
  addPedidoBorrado(factura);
  // Marcar en Sheets como backup (best effort, no-cors)
...
function getInformeRows(incluirEnviados = false) {
  ...
  Object.entries(despachados).forEach(([factura, d]) => {
    d.items.forEach((item, idx) => {
      if (item.Faltante > 0) {
        if (!incluirEnviados && item.Enviado) return; // ocultar enviados
        rows.push({
...
async function borrarFaltante(factura, sku) {
  ...
  // localStorage PRIMERO — persiste siempre
  addFaltanteBorrado(factura, sku);
  // Marcar en Sheets como backup (best effort)
