...
let registros    = {};   // { factura: { dir, counts, fecha, despachado } } — último registro guardado en "registro"
let despachadosFacturas = new Set(); // facturas ya despachadas (pasan a Historial)
let despachados  = {};   // { factura: { items: [{Sku,NombreProducto,Cantidad,Despachado,Faltante,Enviado,FechaEnvio}], Cliente, Sucursal, Telefono, Fecha, dir, cajas, fechaDespacho } }
let despachosLog = [];   // lista plana de despachos para el informe por rango de fechas
let rawPedidos   = [];
...
function doLogin() {
  ...
  // Continuar con la carga normal de la app
...
document.addEventListener('DOMContentLoaded', () => {
  // El login siempre se pide al abrir — no hay sesión persistente
  document.getElementById('loginModal').classList.add('active');
});
...
async function loadAll() {
  ...
  try {
    ...
    despachosLog = [];
    const despachosVistos = new Set(); // para no duplicar entradas del log por factura
    rRows.forEach(r => {
      const factura     = r[0] || '';
      ...
      if (despachado) {
        ...
        // Una sola entrada de log por factura (la fila de despacho se repite por cada sku, tomamos la primera)
...
function buildPedidoCard(p, isHistorial) {
  ...
  const est   = despachado?'completado':pr.pct>0?'en-progreso':'pendiente';
  const badge = despachado?'🚚 Despachado':pr.pct>0?'↻ En progreso':'○ Pendiente';
...
function renderProductosTable() {
  ...
  body.innerHTML = currentPedido.items.map((item) => {
    ...
    // Casilla "Bonificado" en TODAS las filas con SKU duplicado (ocurrencia 2+)
...
async function despacharPedido() {
  ...
  // 1. Guardar TODO en hoja registro (historial completo de lo que se despachó) + marca Despachado=TRUE + Despachador
  const regRows = p.items.map(item => [
    factura, p.Fecha, p.Observacion, p.Cliente, p.Sucursal, dir, p.Telefono,
    item.Sku, item.NombreProducto, item.Cantidad, counts[item.Sku]||0, ahora, cajas, 'TRUE', currentUser
  ]);

  // 2. Guardar SOLO los faltantes en hoja despachos (para el informe)
  // Cols: Factura,Fecha,Cliente,Sucursal,Telefono,Direccion,Sku,NomProd,CantPedida,CantDespachada,Faltante,FechaDespacho,Cajas,Enviado,FechaEnvio
...
function printSticker() {
  ...
  for (let caja = 1; caja <= cajas; caja++) {
    ...
    pages.push(`
    <div class="sp" ${!esUltima ? 'style="page-break-after:always;margin-bottom:3mm"' : ''}>
      ...
      <div class="dest">
        ...
        <div class="row"><span class="lbl">Tel</span><span class="val">${escHtml(p.Telefono)}</span></div>
  ...
  <style>
    @page { size:100mm 90mm; margin:0; }
    *     { box-sizing:border-box; margin:0; padding:0; font-family:Arial,sans-serif; }
    body  { background:white; }
    .sp {
      width:100mm; height:90mm; padding:3mm 3.5mm;
      display:flex; flex-direction:column; justify-content:space-between; overflow:hidden;
    }
    /* Remitente */
    .rem       { display:flex; align-items:center; gap:2mm; }
    .logo      { width:11mm; height:11mm; object-fit:contain; flex-shrink:0; }
    .rem-info  { display:flex; flex-direction:column; gap:0.3mm; }
    .rem-nom   { font-size:7pt; font-weight:900; color:#1a2744; }
    .rem-d     { font-size:5pt; color:#475569; }
    /* Separador */
    .sep { border:none; border-top:0.3mm solid #cbd5e1; margin:1.2mm 0; }
    /* Destinatario */
    .dest      { flex:1; }
    .dest-lbl  { font-size:4.5pt; font-weight:700; color:#94a3b8; text-transform:uppercase; letter-spacing:0.8px; margin-bottom:0.4mm; }
    .fnum      { font-size:14pt; font-weight:900; color:#1a2744; line-height:1; margin-bottom:0.5mm; }
    .cli       { font-size:7pt; font-weight:700; color:#1a2744; margin-bottom:0.8mm; }
    .row       { display:flex; justify-content:space-between; align-items:baseline; margin-bottom:0.4mm; }
    .lbl       { font-size:5pt; color:#94a3b8; text-transform:uppercase; font-weight:600; flex-shrink:0; }
    .val       { font-size:6.5pt; font-weight:700; color:#1a2744; text-align:right; max-width:55mm; word-break:break-word; }
    .dir       { font-size:6.5pt; font-weight:700; color:#0f172a; line-height:1.2; margin-top:0.5mm; }
    /* Contador caja */
    .bot  { display:flex; flex-direction:column; align-items:center; }
    .cnt  { display:flex; align-items:baseline; gap:0.5mm; line-height:1; }
    .ca   { font-size:17pt; font-weight:900; color:#1a2744; }
    .cs   { font-size:11pt; font-weight:700; color:#94a3b8; }
    .ct   { font-size:11pt; font-weight:700; color:#64748b; }
    .cl   { font-size:5.5pt; font-weight:700; color:#1a2744; text-transform:uppercase; letter-spacing:0.5px; margin-top:0.4mm; }
    .ts   { font-size:4.5pt; color:#94a3b8; margin-top:0.4mm; }
  </style></head>
  <body>${pages.join('')}</body></html>`;

  const win = window.open('','_blank',`width=420,height=${Math.min(cajas,3)*400}`);
