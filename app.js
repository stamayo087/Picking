...
async function borrarPedido(factura) {
  const clave = prompt(`¿Borrar el pedido ${factura}?\n\nEsta acción lo eliminará de la lista.\nIngresa la clave de autorización:`);
  if (clave === null) return;
  if (clave.trim() !== '901143584') {
  ...
  try {
    // Marcar con X en la hoja Pedidos (columna L = índice 11)
    await scriptPost('marcarEliminado', [['Pedidos', 'A', factura, 'L', 'X']]);
  } catch(e) { /* continuar */ }
...
async function borrarFaltante(factura, itemIndex) {
  const clave = prompt('Ingresa la clave de autorización para borrar este faltante:');
  if (clave === null) return;
  if (clave.trim() !== '901143584') {
    showToast('⛔ Clave incorrecta — borrado cancelado', 'error');
    return;
  ...
  try {
    // Marcar con X en la hoja despachos (col O = Eliminado)
    // Se identifica por Factura + SKU
    await scriptPost('marcarEliminadoFaltante', [['despachos', factura, item.Sku, 'O', 'X']]);
  } catch(e) { /* continuar */ }

  // Quitar de memoria — faltante = 0 lo excluye del informe
  item.Faltante = 0;
  hideLoading();
  renderInforme();
