...
function procesarDespachos(despachosData) {
  const dRows = (despachosData.values || []).slice(1).filter(r => {
    if (!r[0]) return false;
    const eliminado = (r[15] || '').toString().trim().toUpperCase(); // col P = Eliminado
    return eliminado !== 'X';
  });
  despachados = {};
  dRows.forEach(r => {
    ...
    despachados[factura].items.push({
      ...
      FechaEnvio: r[14]||''   // col O = FechaEnvio
    });
  });
}
...
async function loadAll() {
  ...
  try {
    ...
    // Procesar hoja Pedidos — filtrar filas marcadas con X en col L (índice 11)
...
async function borrarPedido(factura) {
  try {
    await pedirClave('Borrar pedido', `Factura: ${factura}`);
  } catch(e) { return; }
  ...
  try {
    ...
    // Si JSONP falla, intentar con no-cors como fallback y esperar
    await scriptPost('marcarEliminado', [['Pedidos', factura, 'L', 'X']]);
    await new Promise(r => setTimeout(r, 2000)); // dar tiempo al Apps Script
  }
...
async function borrarFaltante(factura, itemIndex) {
  ...
  try {
    await scriptPostConfirm('marcarEliminadoFaltante', [[factura, item.Sku, 'P', 'X']]);
  } catch(e) {
    await scriptPost('marcarEliminadoFaltante', [[factura, item.Sku, 'P', 'X']]);
