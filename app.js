...
async function borrarPedido(factura) {
  ...
  if (!confirm(`¿Confirmar borrado del pedido ${factura}? Esta acción no se puede deshacer.`)) return;

  showLoading('Borrando pedido...');
  try {
    await scriptPost('borrarPedido', [[factura]]);
  } catch(e) { /* continuar aunque falle el script */ }
...
async function borrarFaltante(factura, itemIndex) {
  ...
  // Marcar como enviado en memoria para sacarlo de la vista
  item.Faltante = 0;
  renderInforme();
  showToast(`✓ Faltante de ${item.NombreProducto} eliminado de la vista`, 'success');
  // Guardar el borrado marcándolo como enviado en Sheets
  item.Enviado = true;
  item.FechaEnvio = new Date().toISOString().slice(0,10);
  await persistEnvioUpdate(factura, item);
}
