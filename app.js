...
async function silentRefresh() {
  try {
    const [registroData, despachosData] = await Promise.all([
      ...
      sheetsGet('despachos!A1:P').catch(() => ({ values: [] }))
    ]);
    const prevDespachadosSize = despachadosFacturas.size;
    const prevDespachados = JSON.stringify(despachados);
    ...
    const hayCambios = hayNuevosDespachos || JSON.stringify(despachados) !== prevDespachados;
    if (hayNuevosDespachos) {
      renderPedidos();
      renderHistorial();
    }
    // Siempre re-renderizar informe en cada refresh para reflejar borrados/enviados
    renderInforme();
...
function procesarDespachos(despachosData) {
  const dRows = (despachosData.values || []).slice(1).filter(r => {
    if (!r[0]) return false;
    const eliminado = (r[15] || '').toString().trim().toUpperCase();
    return eliminado !== 'X';
  });
  despachados = {};
  dRows.forEach(r => {
    ...
    const sku = r[6]||'';
    // Filtrar también por lista negra de localStorage
    if (esFaltanteBorrado(factura, sku)) return;
    despachados[factura].items.push({
      Sku: sku, NombreProducto: r[7]||'',
...
// Versión con confirmación via JSONP (para operaciones críticas)
async function scriptPostConfirm(action, rows) {
  return new Promise((resolve, reject) => {
    const cb   = 'cb_' + Date.now();
    const data = encodeURIComponent(JSON.stringify({ action, rows }));
    const url  = `${APPS_SCRIPT_URL}?callback=${cb}&payload=${data}`;
    ...
    window[cb] = (resp) => {
      ...
      delete window[cb];
      script.remove();
      resp.ok ? resolve(resp) : reject(new Error(resp.error || 'Error'));
    };
    script.src = url;
...
// ===== MODAL DE CLAVE CON CAMPO PASSWORD =====
let _claveResolve = null;
let _claveReject  = null;
let _claveCorrecta = '901143584';

function pedirClave(titulo = 'Autorización requerida', descripcion = '') {
  return new Promise((resolve, reject) => {
    _claveResolve = resolve;
    _claveReject  = reject;
    document.getElementById('claveModalTitle').textContent = titulo;
    document.getElementById('claveModalDesc').textContent  = descripcion;
    document.getElementById('claveInput').value = '';
    document.getElementById('claveError').classList.add('hidden');
    document.getElementById('claveModal').classList.add('active');
    setTimeout(() => document.getElementById('claveInput').focus(), 100);
  });
}

function claveModalConfirm() {
  const val = document.getElementById('claveInput').value.trim();
  if (val !== _claveCorrecta) {
    document.getElementById('claveError').classList.remove('hidden');
    document.getElementById('claveInput').value = '';
    document.getElementById('claveInput').focus();
    return;
  }
  document.getElementById('claveModal').classList.remove('active');
  if (_claveResolve) { _claveResolve(true); _claveResolve = null; }
}

function claveModalCancel() {
  document.getElementById('claveModal').classList.remove('active');
  document.getElementById('claveInput').value = '';
  document.getElementById('claveError').classList.add('hidden');
  if (_claveReject) { _claveReject(new Error('cancelado')); _claveReject = null; }
}

// ===== LISTAS NEGRAS (localStorage + memoria de sesión) =====
const BORRADOS_PEDIDOS_KEY   = 'pedidos_borrados';
const BORRADOS_FALTANTES_KEY = 'faltantes_borrados';

// Caches en memoria — sobreviven silentRefresh sin releer localStorage
const _pedidosBorradosCache   = new Set();
const _faltantesBorradosCache = []; // [{f, s}]
let   _cachesCargados = false;

function _cargarCaches() {
  if (_cachesCargados) return;
  try {
    const p = JSON.parse(localStorage.getItem(BORRADOS_PEDIDOS_KEY) || '[]');
    p.forEach(f => _pedidosBorradosCache.add(f));
  } catch(e) {}
  try {
    const fs = JSON.parse(localStorage.getItem(BORRADOS_FALTANTES_KEY) || '[]');
    fs.forEach(x => _faltantesBorradosCache.push(x));
  } catch(e) {}
  _cachesCargados = true;
}

function getPedidosBorrados() {
  _cargarCaches();
  return _pedidosBorradosCache;
}
function addPedidoBorrado(factura) {
  _cargarCaches();
  _pedidosBorradosCache.add(factura.toString().trim());
  try {
    localStorage.setItem(BORRADOS_PEDIDOS_KEY, JSON.stringify([..._pedidosBorradosCache]));
  } catch(e) {}
}

function getFaltantesBorrados() {
  _cargarCaches();
  return _faltantesBorradosCache;
}
function addFaltanteBorrado(factura, sku) {
  _cargarCaches();
  const fNorm = factura.toString().trim();
  const sNorm = sku.toString().trim();
  if (!_faltantesBorradosCache.find(x => x.f === fNorm && x.s === sNorm)) {
    _faltantesBorradosCache.push({ f: fNorm, s: sNorm });
    try {
      localStorage.setItem(BORRADOS_FALTANTES_KEY, JSON.stringify(_faltantesBorradosCache));
    } catch(e) {}
  }
}
function esFaltanteBorrado(factura, sku) {
  _cargarCaches();
  const fNorm = factura.toString().trim();
  const sNorm = sku.toString().trim();
  return _faltantesBorradosCache.some(x => x.f === fNorm && x.s === sNorm);
}
async function loadAll() {
  ...
  try {
    ...
    const [pedidosData, registroData, productosData, despachosData] = await Promise.all([
      sheetsGet('Pedidos!A1:L'),                                        // L = columna Eliminado
      sheetsGet('registro!A1:O').catch(() => ({ values: [] })),
      sheetsGet('Productos!A1:Z').catch(() => ({ values: [] })),
      sheetsGet('despachos!A1:P').catch(() => ({ values: [] }))         // O = columna Eliminado
    ]);

    // ===== CARGA COMPLETA =====
    // Procesar hoja Pedidos — filtrar X en col L Y filtrar lista negra de localStorage
    const pedidosBorrados = getPedidosBorrados();
    const pRows = pedidosData.values || [];
    if (pRows.length >= 2) {
      const dataRows = pRows.slice(1).filter(r => {
        if (!r.some(c => c)) return false;
        const factura   = (r[0] || '').toString().trim();
        const eliminado = (r[11] || '').toString().trim().toUpperCase();
        return eliminado !== 'X' && !pedidosBorrados.has(factura);
      });
      allPedidos = groupPedidos(pRows[0], dataRows);
    } else { allPedidos = []; }
...
function renderPedidos() {
  ...
  const borrados = getPedidosBorrados();
  const todosVisibles = allPedidos.filter(p => !borrados.has(p.Factura.toString().trim()));
  const pendientes     = todosVisibles.filter(p => !despachadosFacturas.has(p.Factura));
  const despachadosCount = todosVisibles.filter(p => despachadosFacturas.has(p.Factura)).length;

  // Poblar filtro sucursal
  const sucursales = [...new Set(todosVisibles.map(p => p.Sucursal).filter(Boolean))];
  ...
  document.getElementById('statTotal').textContent     = todosVisibles.length;
...
function renderHistorial() {
  ...
  const borrados = getPedidosBorrados();
  const despachadosList = allPedidos.filter(p =>
    despachadosFacturas.has(p.Factura) && !borrados.has(p.Factura.toString().trim())
  );
...
async function editarPedidoDesdeHistorial(factura) {
  try { await pedirClave('Editar pedido despachado', `Factura: ${factura}`); } catch(e) { return; }
    ...
    <div class="card-footer">
      ...
      ${isHistorial
        ...
        : `<div class="card-footer-actions">
             ...
             <button class="btn-borrar-pedido" onclick="event.stopPropagation();borrarPedido('${escHtml(p.Factura)}')">🗑️</button>
           </div>`}
    </div>
  </div>`;
}
...
async function borrarPedido(factura) {
  try { await pedirClave('Borrar pedido', `Factura: ${factura}`); } catch(e) { return; }
  if (!confirm(`¿Confirmar borrado del pedido ${factura}?\nEsta acción no se puede deshacer.`)) return;

  showLoading('Borrando pedido...');

  // 1. Guardar en localStorage (persiste entre recargas en este dispositivo)
  addPedidoBorrado(factura);

  // 2. Marcar X en Google Sheets via JSONP (confirma escritura real)
  try {
    await scriptPostConfirm('marcarEliminado', [['Pedidos', factura, 'L', 'X']]);
  } catch(e) {
    // Fallback con POST normal
    scriptPost('marcarEliminado', [['Pedidos', factura, 'L', 'X']]).catch(()=>{});
  }

  // 3. Borrar de memoria local
  allPedidos = allPedidos.filter(p => p.Factura !== factura);
  clearScanLocal(factura);
  delete scanCounts[factura];
  delete registros[factura];
  despachadosFacturas.delete(factura);

  hideLoading();
  showToast(`✓ Pedido ${factura} eliminado`, 'success');
  renderPedidos();
  renderHistorial();
  updateConsecutivoDisplay();
}
...
async function borrarItemDuplicado(itemKey) {
  if (!currentPedido) return;
  const item = currentPedido.items.find(i => i.itemKey === itemKey);
  if (!item) return;

  // Pedir clave con modal
  try {
    await pedirClave('Borrar producto duplicado', `${item.NombreProducto} (SKU: ${item.Sku})`);
  } catch(e) { return; }
  if (!confirm(`¿Confirmar borrado?\nProducto: ${item.NombreProducto}\nEsta acción solo afecta la sesión actual.`)) return;
...
function getInformeRows(incluirEnviados = false) {
  const rows = [];
  Object.entries(despachados).forEach(([factura, d]) => {
    d.items.forEach((item) => {
      if (item.Faltante > 0) {
        if (!incluirEnviados && item.Enviado) return;
        // Filtrar faltantes borrados en tiempo de render
        if (esFaltanteBorrado(factura, item.Sku)) return;
        rows.push({
...
function renderInforme() {
  const allRows     = getInformeRows(true);  // todos (incluye enviados) para stats
  const visibleRows = getInformeRows(false); // solo pendientes para la tabla
  const q = (document.getElementById('searchInforme')?.value||'').toLowerCase();
  const rows = visibleRows
    .filter(r => !q || r.Factura.toLowerCase().includes(q) || r.Cliente.toLowerCase().includes(q) ||
      r.Sku.toLowerCase().includes(q) || r.NombreProducto.toLowerCase().includes(q))
    .sort((a, b) => a.Factura.localeCompare(b.Factura, undefined, { numeric: true }));

  const badge = document.getElementById('informeBadge');
  const facturasPendientes = new Set(visibleRows.map(r=>r.Factura)).size;
  if (facturasPendientes>0) { badge.textContent=facturasPendientes; badge.style.display='inline-flex'; }
  else badge.style.display='none';

  document.getElementById('informeSubtitle').textContent =
    `${rows.length} línea(s) pendiente(s) en ${new Set(rows.map(r=>r.Factura)).size} pedido(s)`;
  ...
  const totalFaltantes = allRows.reduce((s,r)=>s+r.Faltante, 0);
  const nFacturas      = new Set(allRows.map(r=>r.Factura)).size;
  const nSkus          = new Set(allRows.map(r=>r.Sku)).size;
  const nEnviados      = allRows.filter(r=>r.Enviado).length;
  ...
  if (!rows.length) {
    table.style.display = 'none';
    empty.classList.remove('hidden');
    empty.querySelector('p').textContent = allRows.length > 0
      ? '✅ Todos los faltantes han sido enviados'
  ...
  body.innerHTML = (() => {
    ...
    rows.forEach(r => {
      // Encabezado de grupo cuando cambia la factura
      if (r.Factura !== facturaActual) {
        facturaActual = r.Factura;
        html += `<tr class="informe-group-header">
          <td colspan="7">
            <span class="informe-factura-badge">📄 Factura ${escHtml(r.Factura)}</span>
            <span class="informe-cliente-badge">${escHtml(r.Cliente)}</span>
            <span class="informe-sucursal-badge">🏪 ${escHtml(r.Sucursal)}</span>
            <span class="informe-tel-badge">📞 ${escHtml(r.Telefono)}</span>
            <span class="informe-dir-badge">📍 ${escHtml(r.Direccion)}</span>
            <span class="informe-fecha-badge">🗓 ${escHtml(r.Fecha)}</span>
          </td>
        </tr>`;
      }
      html += `<tr>
        ...
        <td style="text-align:center;padding:2px">
          <input type="number" min="0" max="${r.CantPedida}" value="${r.CantDespachada}"
            class="qty-manual" style="width:60px;text-align:center"
            onchange="setCantDespachada('${escHtml(r.Factura)}','${escHtml(r.Sku)}',this.value,${r.CantPedida})"
            onclick="this.select()" />
        </td>
        <td style="text-align:center"><span class="faltante-badge" id="faltante-${escHtml(r.Factura)}-${escHtml(r.Sku)}">${r.Faltante}</span></td>
        <td style="text-align:center">
          <input type="checkbox" class="enviado-check" ${r.Enviado?'checked':''}
            onchange="toggleEnviado('${escHtml(r.Factura)}','${escHtml(r.Sku)}',this.checked)" />
        </td>
        <td>
          <input type="date" class="fecha-envio-input" value="${r.FechaEnvio ? toDateInputValue(r.FechaEnvio) : ''}"
            onchange="setFechaEnvio('${escHtml(r.Factura)}','${escHtml(r.Sku)}',this.value)" />
        </td>
        <td>
          <button class="btn-borrar-dup" title="Borrar este faltante"
            onclick="borrarFaltante('${escHtml(r.Factura)}','${escHtml(r.Sku)}')">✕</button>
        </td>
      </tr>`;
    });
    return html;
...
async function toggleEnviado(factura, sku, checked) {
  if (!despachados[factura]) return;
  const item = despachados[factura].items.find(i => i.Sku.toString().trim() === sku.toString().trim());
  if (!item) return;
  item.Enviado = checked;
  if (checked && !item.FechaEnvio) {
    item.FechaEnvio = new Date().toISOString().slice(0,10);
  }
  if (checked) showToast(`✅ ${item.NombreProducto} marcado como enviado`, 'success');
  renderInforme();
  await persistEnvioUpdate(factura, item);
}

async function borrarFaltante(factura, sku) {
  try { await pedirClave('Borrar faltante', `Factura: ${factura} — SKU: ${sku}`); } catch(e) { return; }
  if (!despachados[factura]) return;
  const item = despachados[factura].items.find(i => i.Sku.toString().trim() === sku.toString().trim());
  if (!item) return;
  if (!confirm(`¿Borrar el faltante?\nProducto: ${item.NombreProducto}\nFactura: ${factura}`)) return;

  showLoading('Borrando faltante...');

  // 1. Guardar en localStorage (persiste entre recargas en este dispositivo)
  addFaltanteBorrado(factura, sku);

  // 2. Marcar X en Google Sheets via JSONP (confirma escritura real)
  try {
    await scriptPostConfirm('marcarEliminadoFaltante', [[factura, sku, 'P', 'X']]);
    showToast(`✓ Faltante eliminado y guardado en Sheets`, 'success');
  } catch(e) {
    // Si JSONP falla, intentar POST normal como fallback
    scriptPost('marcarEliminadoFaltante', [[factura, sku, 'P', 'X']]).catch(()=>{});
    showToast(`✓ Faltante de ${item.NombreProducto} eliminado`, 'success');
  }
...
async function setFechaEnvio(factura, sku, value) {
  if (!despachados[factura]) return;
  const item = despachados[factura].items.find(i => i.Sku.toString().trim() === sku.toString().trim());
  if (!item) return;
...
async function setCantDespachada(factura, sku, value, cantPedida) {
  if (!despachados[factura]) return;
  const item = despachados[factura].items.find(i => i.Sku.toString().trim() === sku.toString().trim());
  if (!item) return;

  const nueva = Math.max(0, Math.min(parseInt(value)||0, cantPedida));
  item.Despachado = nueva;
  item.Faltante   = Math.max(0, cantPedida - nueva);

  // Actualizar badge de faltante en pantalla sin re-renderizar toda la tabla
  const badge = document.getElementById(`faltante-${factura}-${sku}`);
  if (badge) badge.textContent = item.Faltante;

  // Si faltante llegó a 0 → ocultar la fila después de un momento
  if (item.Faltante === 0) {
    setTimeout(() => renderInforme(), 800);
  }

  // Guardar en Sheets via Apps Script
  try {
    await scriptPostConfirm('actualizarDespachado', [[factura, sku, nueva, item.Faltante]]);
    showToast(`✓ Cantidad actualizada — Faltante: ${item.Faltante}`, 'success');
  } catch(e) {
    scriptPost('actualizarDespachado', [[factura, sku, nueva, item.Faltante]]).catch(()=>{});
    showToast(`✓ Cantidad actualizada — Faltante: ${item.Faltante}`, 'success');
  }
}
...
function exportInformeCSV() {
  const rows = getInformeRows(true); // exportar todo incluyendo enviados
