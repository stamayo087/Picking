...
async function loadAll() {
  ...
  try {
    ...
    // Procesar hoja Productos — mapa CodBarras -> UndVenta
    ...
    if (prodRows.length >= 2) {
      ...
      if (iCod > -1 && iUnd > -1) {
        prodRows.slice(1).forEach(r => {
          ...
          if (cod) productosMap[cod.toLowerCase()] = und;
        });
      }
    }
...
function processScan(sku) {
  ...
  const multiplicador = getUndVenta(item.Sku); // desde hoja Productos (CodBarras->UndVenta), default 1
  scanCounts[f][item.Sku] = (scanCounts[f][item.Sku]||0) + multiplicador;
  const nuevo=scanCounts[f][item.Sku], pedido=item.Cantidad;
  flashRow(item.Sku);

  const sufijoMult = multiplicador > 1 ? ` (x${multiplicador} und/venta)` : '';
  if(nuevo===pedido)    showToast('✓ '+item.NombreProducto+' — COMPLETO'+sufijoMult,'success');
  else if(nuevo>pedido) showToast('⚡ '+item.NombreProducto+' — Exceso: '+nuevo+'/'+pedido+sufijoMult,'warning');
  else                  showToast('→ '+item.NombreProducto+': '+nuevo+'/'+pedido+sufijoMult,'');
  renderProductosTable();
}
